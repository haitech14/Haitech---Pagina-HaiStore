...
import {
  ...
  COMPATIBLE_BUSHINGS_INVENTORY_LABELS,
  COMPATIBLE_BUSHINGS_SUBCATEGORY_ID,
  COMPATIBLE_BUSHINGS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-bushings.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeBocinasExcel } from './export-generico-bocinas-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_BUSHINGS_SUBCATEGORY_ID,
  name: 'Bocinas',
  slug: COMPATIBLE_BUSHINGS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_BUSHINGS_INVENTORY_LABELS,
};
...
/** @param {import('../data/seeds/generico-bocinas-catalog.mjs').GENERICO_BOCINAS_CATALOG[number]} entry */
...
async function ensureBocinasSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Bocinas compatibles RICOH',
  });
}

/** @param {import('../data/seeds/generico-bocinas-catalog.mjs').GENERICO_BOCINAS_CATALOG[number]} entry */
function buildBushingProduct(entry) {
  const id = `generico-boc-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildBocinaProductName({ title: entry.title, models: entry.models });
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_BUSHINGS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Bocinas' },
      { name: 'Modelo de equipo', value: entry.models || entry.title },
...
async function main() {
  ...
  await ensureBocinasSubcategory();

  const imported = [];
  const imageCache = new Map();

  for (const entry of GENERICO_BOCINAS_CATALOG) {
    ...
    const product = buildBushingProduct(entry);
    await assignProductImages([product], imageCache.get(cacheKey), dryRun);
    imported.push(product);
    console.log(`✓ ${entry.slug} → ${product.name}`);
  }

  const excelPath = writeBocinasExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación bocinas: ${created} nuevos, ${updated} actualizados.`);
