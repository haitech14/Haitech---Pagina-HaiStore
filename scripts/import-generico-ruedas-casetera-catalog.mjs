...
import {
  ...
  COMPATIBLE_CASSETTE_WHEELS_INVENTORY_LABELS,
  COMPATIBLE_CASSETTE_WHEELS_SUBCATEGORY_ID,
  COMPATIBLE_CASSETTE_WHEELS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-cassette-wheels.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeRuedasCaseteraExcel } from './export-generico-ruedas-casetera-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_CASSETTE_WHEELS_SUBCATEGORY_ID,
  name: 'Ruedas de Casetera',
  slug: COMPATIBLE_CASSETTE_WHEELS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_CASSETTE_WHEELS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/generico-ruedas-casetera-catalog.mjs').GENERICO_RUEDAS_CASETERA_CATALOG[number]} entry
 */
...
async function ensureRuedasCaseteraSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Kits de ruedas de casetera compatibles Genérico',
  });
}
 ...
 * @param {import('../data/seeds/generico-ruedas-casetera-catalog.mjs').GENERICO_RUEDAS_CASETERA_CATALOG[number]} entry
 */
function buildCassetteWheelProduct(entry) {
  const id = `generico-rcas-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildRuedasCaseteraProductName({
    title: entry.title,
    models: entry.models,
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_CASSETTE_WHEELS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Ruedas de Casetera' },
      {
        name: 'Modelo de equipo',
        value: entry.models || entry.title.replace(/^(?:KITS?\s+DE\s+RUEDAS\s+DE\s+CASETERA|RUEDA\s+DE\s+CASETERA)\s*/i, ''),
      },
...
async function main() {
  ...
  await ensureRuedasCaseteraSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of GENERICO_RUEDAS_CASETERA_CATALOG) {
    ...
    const product = buildCassetteWheelProduct(entry);
  ...
  const excelPath = writeRuedasCaseteraExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación ruedas de casetera: ${created} nuevos, ${updated} actualizados.`);
