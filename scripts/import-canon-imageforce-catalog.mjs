...
import { CANON_IMAGEFORCE_CATALOG } from '../data/seeds/canon-imageforce-catalog.mjs';
...
import {
  buildCanonImageForceDescription,
  buildCanonImageForceName,
  CANON_BRAND,
  CANON_IMAGEFORCE_RESOLUTION,
  CATEGORY_CANON_MF_NUEVAS,
} from '../server/lib/canon-imageforce.js';
...
/** @param {import('../data/seeds/canon-imageforce-catalog.mjs').CANON_IMAGEFORCE_CATALOG[number]} entry */
...
function buildProduct(entry) {
  ...
  const name = buildCanonImageForceName({ model: entry.model });
  const description = buildCanonImageForceDescription(entry);
  ...
  const attributes = [
    ...
    { name: 'Resolución', value: CANON_IMAGEFORCE_RESOLUTION },
  ];
...
async function main() {
  ...
  for (const entry of CANON_IMAGEFORCE_CATALOG) {
  ...
  console.log(`\nImportación Canon ImageForce completada: ${created} nuevos, ${updated} actualizados.`);
}
