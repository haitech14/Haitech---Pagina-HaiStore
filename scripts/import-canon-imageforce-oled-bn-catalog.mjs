...
import { CANON_IMAGEFORCE_OLED_BN_CATALOG } from '../data/seeds/canon-imageforce-oled-bn-catalog.mjs';
...
import {
  buildCanonImageForceOledBnDescription,
  buildCanonImageForceOledBnName,
  CANON_BRAND,
  CATEGORY_CANON_MF_NUEVAS,
} from '../server/lib/canon-imageforce-oled-bn.js';
...
/** @param {import('../data/seeds/canon-imageforce-oled-bn-catalog.mjs').CANON_IMAGEFORCE_OLED_BN_CATALOG[number]} entry */
...
function buildProduct(entry) {
  ...
  const name = buildCanonImageForceOledBnName({ model: entry.model });
  const description = buildCanonImageForceOledBnDescription(entry);

  /** @type {{ name: string; value: string }[]} */
  const attributes = [
    { name: 'Color', value: 'B/N' },
    { name: 'Modelo', value: entry.model },
    { name: 'Tecnología', value: 'Láser OLED' },
...
async function main() {
  ...
  for (const entry of CANON_IMAGEFORCE_OLED_BN_CATALOG) {
  ...
  console.log(
    `\nImportación Canon ImageForce OLED B/N completada: ${created} nuevos, ${updated} actualizados.`,
  );
}
