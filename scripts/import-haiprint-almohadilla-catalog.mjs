...
import {
  ...
  COMPATIBLE_PADS_INVENTORY_LABELS,
  COMPATIBLE_PADS_SUBCATEGORY_ID,
  COMPATIBLE_PADS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-pads.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeAlmohadillaExcel } from './export-haiprint-almohadilla-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_PADS_SUBCATEGORY_ID,
  name: 'Almohadilla',
  slug: COMPATIBLE_PADS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_PADS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/haiprint-almohadilla-catalog.mjs').HAIPRINT_ALMOHADILLA_CATALOG[number]} entry
 */
...
async function ensureAlmohadillaSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Almohadillas compatibles HaiPrint',
  });
}
...
function buildPadProduct(entry) {
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_PADS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Almohadilla' },
      { name: 'Modelo de equipo', value: entry.models || entry.title.replace(/^ALMOHADILLA\s*/i, '') },
...
async function main() {
  ...
  await ensureAlmohadillaSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of HAIPRINT_ALMOHADILLA_CATALOG) {
    ...
    const product = buildPadProduct(entry);
  ...
  const excelPath = writeAlmohadillaExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación almohadilla: ${created} nuevos, ${updated} actualizados.`);
