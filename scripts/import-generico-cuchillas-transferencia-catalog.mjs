...
import {
  ...
  COMPATIBLE_TRANSFER_BLADES_INVENTORY_LABELS,
  COMPATIBLE_TRANSFER_BLADES_SUBCATEGORY_ID,
  COMPATIBLE_TRANSFER_BLADES_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-transfer-blades.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeCuchillasTransferenciaExcel } from './export-generico-cuchillas-transferencia-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_TRANSFER_BLADES_SUBCATEGORY_ID,
  name: 'Cuchillas de Transferencia',
  slug: COMPATIBLE_TRANSFER_BLADES_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_TRANSFER_BLADES_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/generico-cuchillas-transferencia-catalog.mjs').GENERICO_CUCHILLAS_TRANSFERENCIA_CATALOG[number]} entry
 */
...
async function ensureCuchillasTransferenciaSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Cuchillas de transferencia compatibles Genérico',
  });
}
 ...
 * @param {import('../data/seeds/generico-cuchillas-transferencia-catalog.mjs').GENERICO_CUCHILLAS_TRANSFERENCIA_CATALOG[number]} entry
 */
function buildTransferBladeProduct(entry) {
  const id = `generico-ct-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildCuchillaTransferenciaProductName({
    title: entry.title,
    models: entry.models,
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_TRANSFER_BLADES,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Cuchilla de Transferencia' },
      {
        name: 'Modelo de equipo',
        value: entry.models || entry.title.replace(/^C\.?\s*DE\s+TRANSF\.?\s*/i, ''),
      },
...
async function main() {
  ...
  await ensureCuchillasTransferenciaSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of GENERICO_CUCHILLAS_TRANSFERENCIA_CATALOG) {
    ...
    const product = buildTransferBladeProduct(entry);
  ...
  const excelPath = writeCuchillasTransferenciaExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación cuchillas de transferencia: ${created} nuevos, ${updated} actualizados.`);
