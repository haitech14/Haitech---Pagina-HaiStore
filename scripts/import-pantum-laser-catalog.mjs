...
import { PANTUM_LASER_CATALOG } from '../data/seeds/pantum-laser-catalog.mjs';
...
import {
  buildPantumLaserDescription,
  buildPantumLaserName,
  CATEGORY_PANTUM_LASER_NUEVAS,
  PANTUM_BRAND,
} from '../server/lib/pantum-laser.js';
...
/** @param {import('../data/seeds/pantum-laser-catalog.mjs').PANTUM_LASER_CATALOG[number]} entry */
...
function buildProduct(entry) {
  ...
  const name = buildPantumLaserName({
    model: entry.model,
    colorMode: entry.colorMode,
    format,
  });
  const description = buildPantumLaserDescription(entry);
  ...
  return normalizeProductInput({
    ...
    description,
    brand: PANTUM_BRAND,
    category: CATEGORY_PANTUM_LASER_NUEVAS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
...
async function main() {
  ...
  for (const entry of PANTUM_LASER_CATALOG) {
  ...
  console.log(`\nImportación Pantum láser completada: ${created} nuevos, ${updated} actualizados.`);
}
