...
import { PANTUM_TERMICA_CATALOG } from '../data/seeds/pantum-termica-catalog.mjs';
...
import {
  buildPantumTermicaDescription,
  buildPantumTermicaName,
  CATEGORY_PANTUM_TERMICA_NUEVAS,
  PANTUM_BRAND,
} from '../server/lib/pantum-termica.js';
...
/** @param {import('../data/seeds/pantum-termica-catalog.mjs').PANTUM_TERMICA_CATALOG[number]} entry */
...
function buildProduct(entry) {
  ...
  const name = buildPantumTermicaName({ model: entry.model });
  const description = buildPantumTermicaDescription(entry);

  /** @type {{ name: string; value: string }[]} */
  const attributes = [
    { name: 'Modelo de equipo', value: entry.model },
    { name: 'Tecnología', value: 'Térmica' },
    { name: 'Ancho de impresión', value: '72 mm' },
  ];

  if (entry.specs.resolution) attributes.push({ name: 'Resolución', value: entry.specs.resolution });
  if (entry.specs.printSpeed) {
    attributes.push({ name: 'Velocidad', value: entry.specs.printSpeed });
  }
  ...
  return normalizeProductInput({
    ...
    description,
    brand: PANTUM_BRAND,
    category: CATEGORY_PANTUM_TERMICA_NUEVAS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
...
async function main() {
  ...
  for (const entry of PANTUM_TERMICA_CATALOG) {
  ...
  console.log(`\nImportación Pantum térmica completada: ${created} nuevos, ${updated} actualizados.`);
}
