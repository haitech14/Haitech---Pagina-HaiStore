...
import { PANTUM_MULTIFUNCIONALES_CATALOG } from '../data/seeds/pantum-multifuncionales-catalog.mjs';
...
import {
  buildPantumMultifuncionalDescription,
  buildPantumMultifuncionalName,
  CATEGORY_PANTUM_MF_NUEVAS,
  extractAdfFromPaper,
  PANTUM_BRAND,
} from '../server/lib/pantum-multifuncionales.js';
...
/** @param {import('../data/seeds/pantum-multifuncionales-catalog.mjs').PANTUM_MULTIFUNCIONALES_CATALOG[number]} entry */
...
function buildProduct(entry) {
  const id = `pantum-${entry.slug}`;
  const format = entry.format ?? 'A4';
  const name = buildPantumMultifuncionalName({
    model: entry.model,
    colorMode: entry.colorMode,
    format,
  });
  const description = buildPantumMultifuncionalDescription(entry);
  const adf = extractAdfFromPaper(entry.specs.paper);

  /** @type {{ name: string; value: string }[]} */
  const attributes = [
    { name: 'Color', value: entry.colorMode === 'color' ? 'Color' : 'B/N' },
    { name: 'Formato papel', value: format },
    { name: 'Modelo de equipo', value: entry.model },
    { name: 'Tecnología', value: 'Láser' },
  ];

  if (entry.specs.speed) attributes.push({ name: 'Velocidad', value: entry.specs.speed });
  if (entry.specs.monthlyVolume) {
    attributes.push({ name: 'Producción', value: entry.specs.monthlyVolume });
  }
  if (entry.specs.memory) attributes.push({ name: 'Memoria', value: entry.specs.memory });
  if (entry.specs.storage) attributes.push({ name: 'Almacenamiento', value: entry.specs.storage });
  if (entry.specs.paperOutput) {
    attributes.push({ name: 'Salida de papel', value: entry.specs.paperOutput });
  }
  if (entry.specs.paper) attributes.push({ name: 'Capacidad bandeja', value: entry.specs.paper });
  if (adf) attributes.push({ name: 'Alimentador (ADF)', value: adf });
  if (entry.connectivity) attributes.push({ name: 'Conectividad', value: entry.connectivity });
  ...
  return normalizeProductInput({
    ...
    description,
    brand: PANTUM_BRAND,
    category: CATEGORY_PANTUM_MF_NUEVAS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
...
async function main() {
  ...
  for (const entry of PANTUM_MULTIFUNCIONALES_CATALOG) {
  ...
  console.log(`\nImportación Pantum completada: ${created} nuevos, ${updated} actualizados.`);
}
