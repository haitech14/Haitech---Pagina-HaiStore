...
import {
  ...
  COMPATIBLE_DEVELOPERS_INVENTORY_LABELS,
  COMPATIBLE_DEVELOPERS_SUBCATEGORY_ID,
  COMPATIBLE_DEVELOPERS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-developers.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeReveladorExcel } from './export-generico-revelador-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_DEVELOPERS_SUBCATEGORY_ID,
  name: 'Revelador',
  slug: COMPATIBLE_DEVELOPERS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_DEVELOPERS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/generico-revelador-catalog.mjs').GENERICO_REVELADOR_CATALOG[number]} entry
 */
...
async function ensureReveladorSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Reveladores compatibles Genérico',
  });
}

/**
 * @param {ReturnType<typeof expandReveladorCatalogEntry>[number]} entry
 */
function buildDeveloperProduct(entry) {
  const id = `generico-rv-${entry.productSlug ?? entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildReveladorProductName({
    title: entry.title,
    models: entry.models,
  ...
  const attributes = [
    { name: 'Tipo', value: 'Revelador' },
    {
      name: 'Modelo de equipo',
      value: entry.models || entry.title.replace(/^REVELADOR\s*/i, '').trim() || entry.models,
    },
    { name: 'Marca compatible', value: entry.compatibleBrand },
    { name: 'Proveedor', value: BRAND },
  ];
  if (entry.colorAttribute ?? entry.color) {
    attributes.push({ name: 'Color', value: entry.colorAttribute ?? entry.color });
  }
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_DEVELOPERS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
...
async function main() {
  ...
  await ensureReveladorSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of GENERICO_REVELADOR_CATALOG) {
    ...
    const products = expandReveladorCatalogEntry(entry).map((variant) => buildDeveloperProduct(variant));

    await assignProductImages(products, png, dryRun);
    for (const product of products) {
      imported.push(product);
      console.log(`✓ ${product.id.replace(/^generico-rv-/, '')} → ${product.name}`);
    }
  }

  const excelPath = writeReveladorExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación revelador: ${created} nuevos, ${updated} actualizados.`);
