...
import { GENERICO_BARRA_LUBRICADORA_CATALOG } from '../data/seeds/generico-barra-lubricadora-catalog.mjs';
import { buildBarraLubricadoraProductName } from '../server/lib/generico-barra-lubricadora.js';
...
import {
  ...
  COMPATIBLE_LUBRICATION_BARS_INVENTORY_LABELS,
  COMPATIBLE_LUBRICATION_BARS_SUBCATEGORY_ID,
  COMPATIBLE_LUBRICATION_BARS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-lubrication-bars.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeBarraLubricadoraExcel } from './export-generico-barra-lubricadora-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_LUBRICATION_BARS_SUBCATEGORY_ID,
  name: 'Barra Lubricadora',
  slug: COMPATIBLE_LUBRICATION_BARS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_LUBRICATION_BARS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/generico-barra-lubricadora-catalog.mjs').GENERICO_BARRA_LUBRICADORA_CATALOG[number]} entry
 */
...
async function ensureBarraLubricadoraSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Barras lubricadoras compatibles Genérico',
  });
}
 ...
 * @param {import('../data/seeds/generico-barra-lubricadora-catalog.mjs').GENERICO_BARRA_LUBRICADORA_CATALOG[number]} entry
 */
function buildLubricationBarProduct(entry) {
  const id = `generico-bl-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildBarraLubricadoraProductName({
    title: entry.title,
    models: entry.models,
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_LUBRICATION_BARS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Barra Lubricadora' },
      {
        name: 'Modelo de equipo',
        value: entry.models || entry.title.replace(/^B\.?\s*LUBRICADORA\s*/i, ''),
      },
...
async function main() {
  ...
  await ensureBarraLubricadoraSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of GENERICO_BARRA_LUBRICADORA_CATALOG) {
    ...
    const product = buildLubricationBarProduct(entry);
  ...
  const excelPath = writeBarraLubricadoraExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación barra lubricadora: ${created} nuevos, ${updated} actualizados.`);
