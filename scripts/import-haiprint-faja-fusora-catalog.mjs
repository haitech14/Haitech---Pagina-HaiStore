...
import {
  ...
  COMPATIBLE_FUSER_FILMS_INVENTORY_LABELS,
  COMPATIBLE_FUSER_FILMS_SUBCATEGORY_ID,
  COMPATIBLE_FUSER_FILMS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-fuser-films.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeFajaFusoraExcel } from './export-haiprint-faja-fusora-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_FUSER_FILMS_SUBCATEGORY_ID,
  name: 'Faja Fusora',
  slug: COMPATIBLE_FUSER_FILMS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_FUSER_FILMS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/haiprint-faja-fusora-catalog.mjs').HAIPRINT_FAJA_FUSORA_CATALOG[number]} entry
 */
...
async function ensureFajaFusoraSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Fajas fusoras compatibles HaiPrint',
  });
}
...
function buildFuserFilmProduct(entry) {
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_FUSER_FILMS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Faja Fusora' },
      { name: 'Modelo de equipo', value: entry.models || entry.title.replace(/^FUSER FILM\s+/i, '') },
...
async function main() {
  ...
  await ensureFajaFusoraSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of HAIPRINT_FAJA_FUSORA_CATALOG) {
    ...
    const product = buildFuserFilmProduct(entry);
  ...
  const excelPath = writeFajaFusoraExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación faja fusora: ${created} nuevos, ${updated} actualizados.`);
