...
import { HAIPRINT_RODILLOS_CALOR_CATALOG } from '../data/seeds/haiprint-rodillos-calor-catalog.mjs';
...
import {
  ...
  COMPATIBLE_HEAT_ROLLERS_INVENTORY_LABELS,
  COMPATIBLE_HEAT_ROLLERS_SUBCATEGORY_ID,
  COMPATIBLE_HEAT_ROLLERS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-heat-rollers.js';
...
const SUBCATEGORY = {
  id: COMPATIBLE_HEAT_ROLLERS_SUBCATEGORY_ID,
  name: 'Rodillo de Calor',
  slug: COMPATIBLE_HEAT_ROLLERS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_HEAT_ROLLERS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/haiprint-rodillos-calor-catalog.mjs').HAIPRINT_RODILLOS_CALOR_CATALOG[number]} entry
 */
 ...
 */
function appendNonRepeatingSubtitle(name, subtitle) {
  ...
  const extra = parts.filter((part) => !nameNorm.includes(normalizeForCompare(part)));
 ...
 */
export function buildRodilloCalorProductName({ title, models }) {
  const modelPart = String(title ?? '')
    .trim()
    .replace(/^RODILLO\s+DE\s+CALOR\s*/i, '')
    .trim();
  const base = `Rodillo de Calor Compatible HaiPrint RICOH ${modelPart}`.replace(/\s{2,}/g, ' ').trim();
  return appendNonRepeatingSubtitle(base, models);
}

async function ensureRodillosCalorSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Rodillos de calor compatibles HaiPrint',
  });
}
...
function buildRodilloProduct(entry) {
  const id = `haiprint-rc-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildRodilloCalorProductName({
    title: entry.title,
    models: entry.models,
  });
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_HEAT_ROLLERS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Rodillo de Calor' },
      { name: 'Modelo de equipo', value: entry.models || entry.title.replace(/^RODILLO DE CALOR\s+/i, '') },
...
async function main() {
  ...
  await ensureRodillosCalorSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of HAIPRINT_RODILLOS_CALOR_CATALOG) {
  ...
  console.log(`\nImportación rodillos de calor completada: ${created} nuevos, ${updated} actualizados.`);
