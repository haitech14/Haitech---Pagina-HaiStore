...
import { INTERCOPY_UNAS_FUSOR_CATALOG } from '../data/seeds/intercopy-unas-fusor-catalog.mjs';
...
import {
  ...
  COMPATIBLE_FUSER_NAILS_INVENTORY_LABELS,
  COMPATIBLE_FUSER_NAILS_SUBCATEGORY_ID,
  COMPATIBLE_FUSER_NAILS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-fuser-nails.js';
...
const SUBCATEGORY = {
  id: COMPATIBLE_FUSER_NAILS_SUBCATEGORY_ID,
  name: 'Uñas de Fusor',
  slug: COMPATIBLE_FUSER_NAILS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_FUSER_NAILS_INVENTORY_LABELS,
};
...
/** @param {import('../data/seeds/intercopy-unas-fusor-catalog.mjs').INTERCOPY_UNAS_FUSOR_CATALOG[number]} entry */
...
export function buildUnasFusorProductName({ models }) {
  const subtitle = String(models ?? '').trim();
  let name = 'Uñas Compatible RICOH';
  if (subtitle) name = `${name} ${subtitle}`;
  return name.replace(/\s{2,}/g, ' ').trim();
}

async function ensureUnasFusorSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Uñas de fusor compatibles Ricoh',
  });
}

/** @param {import('../data/seeds/intercopy-unas-fusor-catalog.mjs').INTERCOPY_UNAS_FUSOR_CATALOG[number]} entry */
function buildProduct(entry) {
  const id = `intercopy-unas-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildUnasFusorProductName({ models: entry.models });
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_FUSER_NAILS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
...
async function main() {
  ...
  await ensureUnasFusorSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];

  for (const entry of INTERCOPY_UNAS_FUSOR_CATALOG) {
  ...
  console.log(`\nImportación uñas de fusor completada: ${created} nuevos, ${updated} actualizados.`);
}
