...
import { HAIPRINT_CILINDROS_CATALOG } from '../data/seeds/haiprint-cilindros-catalog.mjs';
...
/**
 * @param {import('../data/seeds/haiprint-cilindros-catalog.mjs').HAIPRINT_CILINDROS_CATALOG[number]} entry
 */
...
export function buildCilindroCompatibleProductName({ title, models, compatibleBrand }) {
  ...
  let name = `Cilindro Compatible HaiPrint ${compatibleBrand} OPC ${opcBody}`;
...
async function ensureCilindrosSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Cilindros OPC compatibles',
  });
}
...
function buildCilindroProduct(entry) {
  const id = `haiprint-cilindro-${entry.slug}`;
...
async function main() {
  ...
  for (const entry of HAIPRINT_CILINDROS_CATALOG) {
  ...
  console.log(`\nImportación cilindros HaiPrint completada: ${created} nuevos, ${updated} actualizados.`);
