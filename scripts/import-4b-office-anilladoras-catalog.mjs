...
import {
  ...
  OFFICE_RING_BINDERS_INVENTORY_LABELS,
  OFFICE_RING_BINDERS_SUBCATEGORY_ID,
  OFFICE_RING_BINDERS_SUBCATEGORY_SLUG,
} from '../shared/office-ring-binders.js';
import { deriveProductSlug } from '../shared/product-slug.js';
import { writeAnilladorasExcel } from './export-4b-office-anilladoras-excel.mjs';
...
/**
 * @param {import('../data/seeds/4b-office-anilladoras-catalog.mjs').OFFICE_4B_ANILLADORAS_CATALOG[number]} entry
...
async function ensureAnilladorasSubcategory() {
  ...
  for (const label of OFFICE_RING_BINDERS_INVENTORY_LABELS) {
  ...
  const existing = categories.find((row) => row.id === OFFICE_RING_BINDERS_SUBCATEGORY_ID);
  if (existing) {
    const labels = new Set([
      ...(existing.inventoryLabels ?? []),
      ...OFFICE_RING_BINDERS_INVENTORY_LABELS,
    ]);
    await updateStoreCategory(OFFICE_RING_BINDERS_SUBCATEGORY_ID, {
      name: 'Anilladoras',
      slug: OFFICE_RING_BINDERS_SUBCATEGORY_SLUG,
  ...
  await createStoreCategory({
    id: OFFICE_RING_BINDERS_SUBCATEGORY_ID,
    name: 'Anilladoras',
    slug: OFFICE_RING_BINDERS_SUBCATEGORY_SLUG,
    parentId: EQUIPOS_OFICINA_PARENT_ID,
    sortOrder: siblings.length,
    inventoryLabels: OFFICE_RING_BINDERS_INVENTORY_LABELS,
    image: '/categories/repuestos.png',
    tagline: 'Anilladoras 4B Office para encuadernación',
  });
}
 ...
 */
function buildRingBinderProduct(entry, media) {
  const id = `4b-an-${entry.slug}`;
  const name = buildAnilladoraProductName({
    code: entry.code,
    perforadoMax: entry.perforadoMax,
    format: entry.format,
  });
  const model = extractAnilladoraModel(entry.code);
  ...
  return normalizeProductInput({
    ...
    brand: entry.brand,
    category: CATEGORY_OFFICE_RING_BINDERS,
    currency: 'USD',
    stock: 0,
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Anilladora' },
      { name: 'Modelo', value: model },
      { name: 'Perforado máx.', value: `${entry.perforadoMax} hojas` },
...
async function main() {
  ...
  await ensureAnilladorasSubcategory();

  const imported = [];

  for (const entry of OFFICE_4B_ANILLADORAS_CATALOG) {
    const id = `4b-an-${entry.slug}`;
    const media = await assignProductImages(entry, id, dryRun);
    const product = buildRingBinderProduct(entry, media);
    imported.push(product);
    console.log(`✓ ${entry.slug} → ${product.name}`);
  }

  const excelPath = writeAnilladorasExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación anilladoras: ${created} nuevos, ${updated} actualizados.`);
