...
import { CANON_IMAGEFORCE_COLOR_CATALOG } from '../data/seeds/canon-imageforce-color-catalog.mjs';
...
import {
  buildCanonImageForceColorDescription,
  buildCanonImageForceColorName,
  CANON_BRAND,
  CATEGORY_CANON_MF_NUEVAS,
} from '../server/lib/canon-imageforce-color.js';
...
/** @param {import('../data/seeds/canon-imageforce-color-catalog.mjs').CANON_IMAGEFORCE_COLOR_CATALOG[number]} entry */
...
function buildProduct(entry) {
  ...
  const name = buildCanonImageForceColorName({ model: entry.model });
  const description = buildCanonImageForceColorDescription(entry);
...
async function main() {
  ...
  for (const entry of CANON_IMAGEFORCE_COLOR_CATALOG) {
  ...
  console.log(
    `\nImportación Canon ImageForce Color completada: ${created} nuevos, ${updated} actualizados.`,
  );
}
