...
import { RICOH_REPUESTO_D6063112 } from '../data/seeds/ricoh-repuesto-d6063112-catalog.mjs';
...
async function assignImages(entry, dryRun) {
  const imageBase = entry.id;
  const heroPath =
    resolveAssetPath(entry.heroAssetToken) ??
    (entry.heroAssetFallback ? resolveAssetPath(entry.heroAssetFallback) : null);
  if (!heroPath) {
    throw new Error(
      `Imagen principal no encontrada (${entry.heroAssetToken}${entry.heroAssetFallback ? ` / ${entry.heroAssetFallback}` : ''})`,
    );
  }
...
async function main() {
  ...
  const entry = RICOH_REPUESTO_D6063112;
