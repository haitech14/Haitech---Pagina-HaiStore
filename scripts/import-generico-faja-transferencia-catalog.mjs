...
import { GENERICO_FAJA_TRANSFERENCIA_CATALOG } from '../data/seeds/generico-faja-transferencia-catalog.mjs';
import { buildFajaTransferenciaProductName } from '../server/lib/generico-faja-transferencia.js';
...
import {
  ...
  COMPATIBLE_TRANSFER_BELTS_INVENTORY_LABELS,
  COMPATIBLE_TRANSFER_BELTS_SUBCATEGORY_ID,
  COMPATIBLE_TRANSFER_BELTS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-transfer-belts.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeFajaTransferenciaExcel } from './export-generico-faja-transferencia-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_TRANSFER_BELTS_SUBCATEGORY_ID,
  name: 'Faja de Transferencia',
  slug: COMPATIBLE_TRANSFER_BELTS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_TRANSFER_BELTS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/generico-faja-transferencia-catalog.mjs').GENERICO_FAJA_TRANSFERENCIA_CATALOG[number]} entry
 */
...
async function ensureFajaTransferenciaSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Fajas de transferencia compatibles Genérico',
  });
}
 ...
 * @param {import('../data/seeds/generico-faja-transferencia-catalog.mjs').GENERICO_FAJA_TRANSFERENCIA_CATALOG[number]} entry
 */
function buildTransferBeltProduct(entry) {
  const id = `generico-ft-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildFajaTransferenciaProductName({
    title: entry.title,
    models: entry.models,
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_TRANSFER_BELTS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Faja de Transferencia' },
      {
        name: 'Modelo de equipo',
        value: entry.models || entry.title.replace(/^FAJA\s+DE\s+TRANSFERENCIA\s*/i, '').trim() || entry.models,
      },
...
async function main() {
  ...
  await ensureFajaTransferenciaSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of GENERICO_FAJA_TRANSFERENCIA_CATALOG) {
    ...
    const product = buildTransferBeltProduct(entry);
  ...
  const excelPath = writeFajaTransferenciaExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación faja de transferencia: ${created} nuevos, ${updated} actualizados.`);
