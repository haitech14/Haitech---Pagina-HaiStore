...
import {
  INTERCOPY_CHIP_COLOR_VARIANTS,
  INTERCOPY_CHIP_TONER_CATALOG,
} from '../data/seeds/intercopy-chip-toner-catalog.mjs';
...
import {
  ...
  COMPATIBLE_TONER_CHIPS_INVENTORY_LABELS,
  COMPATIBLE_TONER_CHIPS_SUBCATEGORY_ID,
  COMPATIBLE_TONER_CHIPS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-toner-chips.js';
...
const SUBCATEGORY = {
  id: COMPATIBLE_TONER_CHIPS_SUBCATEGORY_ID,
  name: 'Chip de Toner',
  slug: COMPATIBLE_TONER_CHIPS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_TONER_CHIPS_INVENTORY_LABELS,
};
...
/** @param {import('../data/seeds/intercopy-chip-toner-catalog.mjs').INTERCOPY_CHIP_TONER_CATALOG[number]} entry */
...
/**
 * @param {{ kind: 'toner' | 'cilindro'; models: string; compatibleBrand: string; colorSuffix?: string }} input
 */
export function buildChipTonerProductName({
  kind,
  models,
  compatibleBrand,
  colorSuffix = '',
}) {
  const tipo = kind === 'cilindro' ? 'cilindro' : 'toner';
  const brand = String(compatibleBrand ?? '').trim();
  const modelsTrim = String(models ?? '').trim();
  let name = `Chip de ${tipo} Compatible ${brand}`;
  if (modelsTrim) {
    name = kind === 'toner' ? `${name} para ${modelsTrim}` : `${name} ${modelsTrim}`;
  }
  if (colorSuffix) name = `${name} ${colorSuffix}`;
  return name.replace(/\s{2,}/g, ' ').trim();
}

/** @param {import('../data/seeds/intercopy-chip-toner-catalog.mjs').INTERCOPY_CHIP_TONER_CATALOG[number]} entry */
function needsFourColors(entry) {
  return Boolean(entry.fourColors);
}

async function ensureChipTonerSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Chips de toner y cilindro compatibles',
  });
}

/** @param {import('../data/seeds/intercopy-chip-toner-catalog.mjs').INTERCOPY_CHIP_TONER_CATALOG[number]} entry */
function buildColorProducts(entry) {
  const chipKind = entry.kind === 'cilindro' ? 'Chip de cilindro' : 'Chip de toner';

  return INTERCOPY_CHIP_COLOR_VARIANTS.map((color) => {
    const id = `intercopy-chip-${entry.slug}-${color.code}`;
    const code = deriveCompatibleTonerNumericCode(id);
    const name = buildChipTonerProductName({
      kind: entry.kind,
      models: entry.models,
    ...
    return normalizeProductInput({
      ...
      description: name,
      brand: BRAND,
      category: CATEGORY_COMPATIBLE_TONER_CHIPS,
      currency: 'USD',
      stock: 0,
      image_url: null,
      gallery: [],
...
function buildMonochromeProduct(entry) {
  const id = `intercopy-chip-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildChipTonerProductName({
    kind: entry.kind,
    models: entry.models,
...
async function main() {
  ...
  await ensureChipTonerSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of INTERCOPY_CHIP_TONER_CATALOG) {
    ...
    const group = needsFourColors(entry) ? buildColorProducts(entry) : [buildMonochromeProduct(entry)];

    await assignProductImages(group, png, dryRun);
    imported.push(...group);

    const mode = needsFourColors(entry) ? '4 colores' : 'monocromático';
    console.log(`✓ ${entry.slug} → ${mode} (${group[0]?.name})`);
  }
  ...
  const staleMonoIds = INTERCOPY_CHIP_TONER_CATALOG.filter((entry) => needsFourColors(entry)).map(
    (entry) => `intercopy-chip-${entry.slug}`,
  );
  for (const staleId of staleMonoIds) {
    if (byId.has(staleId)) {
      byId.delete(staleId);
  ...
  const colorRefs = INTERCOPY_CHIP_TONER_CATALOG.filter((entry) => needsFourColors(entry)).length;
  const monoRefs = INTERCOPY_CHIP_TONER_CATALOG.length - colorRefs;

  console.log(`\nImportación chips de toner completada: ${created} nuevos, ${updated} actualizados.`);
  console.log(
