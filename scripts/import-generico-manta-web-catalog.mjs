...
import {
  ...
  COMPATIBLE_CLEANING_WEBS_INVENTORY_LABELS,
  COMPATIBLE_CLEANING_WEBS_SUBCATEGORY_ID,
  COMPATIBLE_CLEANING_WEBS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-cleaning-webs.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeMantaWebExcel } from './export-generico-manta-web-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_CLEANING_WEBS_SUBCATEGORY_ID,
  name: 'Manta Web',
  slug: COMPATIBLE_CLEANING_WEBS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_CLEANING_WEBS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/generico-manta-web-catalog.mjs').GENERICO_MANTA_WEB_CATALOG[number]} entry
 */
...
async function ensureMantaWebSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Mantas web compatibles Genérico',
  });
}
...
function buildCleaningWebProduct(entry) {
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_CLEANING_WEBS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Manta Web' },
      {
        name: 'Modelo de equipo',
        value: entry.models || entry.title.replace(/^MANTA\s+WEB\s*/i, '').trim() || entry.models,
      },
...
async function main() {
  ...
  await ensureMantaWebSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of GENERICO_MANTA_WEB_CATALOG) {
    ...
    const product = buildCleaningWebProduct(entry);
  ...
  const excelPath = writeMantaWebExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación manta web: ${created} nuevos, ${updated} actualizados.`);
