...
import { CANON_MULTIFUNCIONALES_CATALOG } from '../data/seeds/canon-multifuncionales-catalog.mjs';
...
import {
  buildCanonMultifuncionalDescription,
  buildCanonMultifuncionalName,
  CANON_BRAND,
  CATEGORY_CANON_MF_NUEVAS,
} from '../server/lib/canon-multifuncionales.js';
...
/** @param {import('../data/seeds/canon-multifuncionales-catalog.mjs').CANON_MULTIFUNCIONALES_CATALOG[number]} entry */
...
function buildProduct(entry) {
  ...
  const name = buildCanonMultifuncionalName({ model: entry.model });
  const description = buildCanonMultifuncionalDescription(entry);

  /** @type {{ name: string; value: string }[]} */
  const attributes = [
    { name: 'Color', value: 'B/N' },
    { name: 'Formato', value: 'A3' },
  ...
  if (entry.specs.adf) attributes.push({ name: 'ADF', value: entry.specs.adf });
  if (entry.specs.paper) attributes.push({ name: 'Capacidad bandeja', value: entry.specs.paper });
  if (entry.specs.touchPanel) {
    attributes.push({ name: 'Panel táctil', value: entry.specs.touchPanel });
  }
...
async function main() {
  ...
  for (const entry of CANON_MULTIFUNCIONALES_CATALOG) {
  ...
  console.log(`\nImportación Canon completada: ${created} nuevos, ${updated} actualizados.`);
}
