...
import { RICOH_TONER_408339 } from '../data/seeds/ricoh-toner-408339-catalog.mjs';
...
function findExistingProduct(byId, entry) {
  return (
    byId.get(String(entry.id)) ??
    [...byId.values()].find((row) => String(row.code ?? '').trim() === String(entry.code))
  );
}
...
function syncCatalogJson(product) {
  ...
  const existing =
    ...
    [...byId.values()].find((row) => String(row.code ?? '') === String(product.code));
...
async function assignImages(entry, dryRun) {
  ...
  const galleryTokens = entry.galleryAssetTokens.filter(
    (token) => token !== entry.heroAssetToken,
  );

  if (dryRun) {
    const gallery = [
      `/products/${imageBase}.webp`,
      ...galleryTokens.map((_, index) => `/products/${imageBase}-${index + 2}.webp`),
    ];
...
async function main() {
  ...
  const entry = RICOH_TONER_408339;
  ...
  console.log(
    `  Precio público: USD ${product.prices.public} (S/ ${(product.prices.public * USD_TO_PEN_SALE).toFixed(2)} ref.)`,
  );
  ...
  const prev = findExistingProduct(byId, entry);
    ...
    normalizeProductInput(
      prev
        ? {
            ...
            suppliers: prev.suppliers?.length ? prev.suppliers : product.suppliers,
          }
        : product,
      prev ?? undefined,
      warehouses,
  ...
  const saved = sorted.find((row) => String(row.id) === String(merged.id)) ?? merged;
  syncCatalogJson(saved);

  console.log(`\nImportación completada: ${prev ? 'fusionado' : 'creado'} (${saved.id}).`);
}
