...
import { GENERICO_ENGRANAJE_PINON_CATALOG } from '../data/seeds/generico-engranaje-pinon-catalog.mjs';
import { buildEngranajePinonProductName } from '../server/lib/generico-engranaje-pinon.js';
...
import {
  ...
  COMPATIBLE_GEARS_INVENTORY_LABELS,
  COMPATIBLE_GEARS_SUBCATEGORY_ID,
  COMPATIBLE_GEARS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-gears.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeEngranajePinonExcel } from './export-generico-engranaje-pinon-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_GEARS_SUBCATEGORY_ID,
  name: 'Engranaje/Piñon',
  slug: COMPATIBLE_GEARS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_GEARS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/generico-engranaje-pinon-catalog.mjs').GENERICO_ENGRANAJE_PINON_CATALOG[number]} entry
 */
...
async function ensureEngranajePinonSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Engranajes y piñones compatibles Genérico',
  });
}
 ...
 * @param {import('../data/seeds/generico-engranaje-pinon-catalog.mjs').GENERICO_ENGRANAJE_PINON_CATALOG[number]} entry
 */
function buildGearProduct(entry) {
  const id = `generico-ep-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildEngranajePinonProductName({
    title: entry.title,
    models: entry.models,
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_GEARS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Engranaje/Piñon' },
      {
        name: 'Modelo de equipo',
        value: entry.models || entry.title.replace(/^ENGRANAJE\s*/i, ''),
      },
...
async function main() {
  ...
  await ensureEngranajePinonSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of GENERICO_ENGRANAJE_PINON_CATALOG) {
    ...
    const product = buildGearProduct(entry);
  ...
  const excelPath = writeEngranajePinonExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación engranaje/piñon: ${created} nuevos, ${updated} actualizados.`);
