...
import {
  ...
  COMPATIBLE_CYLINDER_BLADES_INVENTORY_LABELS,
  COMPATIBLE_CYLINDER_BLADES_SUBCATEGORY_ID,
  COMPATIBLE_CYLINDER_BLADES_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-cylinder-blades.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeCuchillasCilindroExcel } from './export-haiprint-cuchillas-cilindro-excel.mjs';
...
const SUBCATEGORY = {
  id: COMPATIBLE_CYLINDER_BLADES_SUBCATEGORY_ID,
  name: 'Cuchillas de Cilindro',
  slug: COMPATIBLE_CYLINDER_BLADES_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_CYLINDER_BLADES_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/haiprint-cuchillas-cilindro-catalog.mjs').HAIPRINT_CUCHILLAS_CILINDRO_CATALOG[number]} entry
 */
...
async function ensureCuchillasCilindroSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Cuchillas de cilindro compatibles HaiPrint',
  });
}
 ...
 * @param {import('../data/seeds/haiprint-cuchillas-cilindro-catalog.mjs').HAIPRINT_CUCHILLAS_CILINDRO_CATALOG[number]} entry
 */
function buildCuchillaProduct(entry) {
  const id = `haiprint-cc-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildCuchillaCilindroProductName({
    title: entry.title,
    models: entry.models,
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_CYLINDER_BLADES,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Cuchilla de Cilindro' },
      { name: 'Modelo de equipo', value: entry.models || entry.title.replace(/^C\.?\s*DE\s+LIMPIEZA\s+/i, '') },
...
async function main() {
  ...
  await ensureCuchillasCilindroSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of HAIPRINT_CUCHILLAS_CILINDRO_CATALOG) {
    ...
    const product = buildCuchillaProduct(entry);
  ...
  const excelPath = writeCuchillasCilindroExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  console.log(`\nImportación cuchillas de cilindro: ${created} nuevos, ${updated} actualizados.`);
