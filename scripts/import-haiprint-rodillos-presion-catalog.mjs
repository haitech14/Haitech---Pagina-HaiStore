...
import { HAIPRINT_RODILLOS_PRESION_CATALOG } from '../data/seeds/haiprint-rodillos-presion-catalog.mjs';
import { SUPPLIER_MICAMERB } from '../server/lib/compatible-toner-excel.js';
import { buildRodilloPresionProductName } from '../server/lib/haiprint-rodillos-presion.js';
...
import {
  ...
  COMPATIBLE_PRESSURE_ROLLERS_INVENTORY_LABELS,
  COMPATIBLE_PRESSURE_ROLLERS_SUBCATEGORY_ID,
  COMPATIBLE_PRESSURE_ROLLERS_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-pressure-rollers.js';
import { deriveCompatibleTonerNumericCode } from '../shared/compatible-toner-product-code.js';
import { writeRodillosPresionExcel } from './export-haiprint-rodillos-presion-excel.mjs';
...
const BRAND = 'HaiPrint';
const SUPPLIER = 'MICAMERB';

const SUBCATEGORY = {
  id: COMPATIBLE_PRESSURE_ROLLERS_SUBCATEGORY_ID,
  name: 'Rodillo de Presión',
  slug: COMPATIBLE_PRESSURE_ROLLERS_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_PRESSURE_ROLLERS_INVENTORY_LABELS,
};
...
/**
 * @param {import('../data/seeds/haiprint-rodillos-presion-catalog.mjs').HAIPRINT_RODILLOS_PRESION_CATALOG[number]} entry
 */
...
async function ensureRodillosPresionSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Rodillos de presión compatibles HaiPrint',
  });
}
 ...
 * @param {import('../data/seeds/haiprint-rodillos-presion-catalog.mjs').HAIPRINT_RODILLOS_PRESION_CATALOG[number]} entry
 */
function buildRodilloProduct(entry) {
  const id = `haiprint-rp-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildRodilloPresionProductName({
    title: entry.title,
    models: entry.models,
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_PRESSURE_ROLLERS,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Rodillo de Presión' },
      { name: 'Modelo de equipo', value: entry.models || entry.title.replace(/^RODILLO DE PRESI[ÓO]N\s+/i, '') },
...
function syncCatalogJson(products, staleIds = []) {
  const catalogPath = path.join(root, 'src', 'data', 'inventory-catalog.json');
  const catalog = JSON.parse(fs.readFileSync(catalogPath, 'utf8'));
  const stale = new Set(staleIds.map(String));
  const validIds = new Set(HAIPRINT_RODILLOS_PRESION_CATALOG.map((entry) => `haiprint-rp-${entry.slug}`));
  const byId = new Map(
    (catalog.products ?? [])
      .filter((product) => {
        const id = String(product.id);
        if (!id.startsWith('haiprint-rp-')) return true;
  ...
  if (stale.size > 0) {
    console.log(`Catálogo estático: ${stale.size} entradas haiprint-rp obsoletas eliminadas.`);
  }
}

async function main() {
  const dryRun = process.argv.includes('--dry-run');
  await ensureRodillosPresionSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];
  const imageCache = new Map();

  for (const entry of HAIPRINT_RODILLOS_PRESION_CATALOG) {
    ...
    const product = buildRodilloProduct(entry);
  ...
  const excelPath = writeRodillosPresionExcel(imported);
  console.log(`\nExcel: ${excelPath}`);
  ...
  for (const [id, product] of byId) {
    if (!id.startsWith('haiprint-rp-') || validIds.has(id)) continue;
    byId.delete(id);
    deletedProductIds.add(id);
    console.log(`✗ eliminado obsoleto: ${id} (${product.name})`);
  }
  ...
  console.log(`\nImportación rodillos de presión: ${created} nuevos, ${updated} actualizados.`);
