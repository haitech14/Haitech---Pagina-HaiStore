...
import { INTERCOPY_PINON_FUSOR_CATALOG } from '../data/seeds/intercopy-pinon-fusor-catalog.mjs';
...
import {
  ...
  COMPATIBLE_FUSER_PINION_INVENTORY_LABELS,
  COMPATIBLE_FUSER_PINION_SUBCATEGORY_ID,
  COMPATIBLE_FUSER_PINION_SUBCATEGORY_SLUG,
  REPUESTOS_COMPATIBLES_PARENT_ID,
} from '../shared/compatible-fuser-pinion.js';
...
const SUBCATEGORY = {
  id: COMPATIBLE_FUSER_PINION_SUBCATEGORY_ID,
  name: 'Piñon de Fusor',
  slug: COMPATIBLE_FUSER_PINION_SUBCATEGORY_SLUG,
  inventoryLabels: COMPATIBLE_FUSER_PINION_INVENTORY_LABELS,
};
...
/** @param {import('../data/seeds/intercopy-pinon-fusor-catalog.mjs').INTERCOPY_PINON_FUSOR_CATALOG[number]} entry */
async function extractCatalogImage(entry) {
  const sourcePath = resolveAssetPath(entry.imageId);
  if (!sourcePath) return null;
  const buffer = await fs.promises.readFile(sourcePath);
  return extractProductPhoto(buffer);
}
...
/** @param {{ models: string }} input */
export function buildPinonFusorProductName({ models }) {
  const subtitle = String(models ?? '').trim();
  let name = 'Piñon para unidad fusora Compatible RICOH';
  if (subtitle) name = `${name} ${subtitle}`;
  return name.replace(/\s{2,}/g, ' ').trim();
}

async function ensurePinonFusorSubcategory() {
  ...
  await createStoreCategory({
    ...
    tagline: 'Piñones de fusor compatibles Ricoh',
  });
}

/** @param {import('../data/seeds/intercopy-pinon-fusor-catalog.mjs').INTERCOPY_PINON_FUSOR_CATALOG[number]} entry */
function buildProduct(entry) {
  const id = `intercopy-pinon-${entry.slug}`;
  const code = deriveCompatibleTonerNumericCode(id);
  const name = buildPinonFusorProductName({ models: entry.models });
  ...
  return normalizeProductInput({
    ...
    description: name,
    brand: BRAND,
    category: CATEGORY_COMPATIBLE_FUSER_PINION,
    currency: 'USD',
    stock: 0,
    image_url: null,
    gallery: [],
    ...
    attributes: normalizeAttributes([
      { name: 'Tipo', value: 'Piñon de fusor' },
...
async function main() {
  ...
  await ensurePinonFusorSubcategory();

  /** @type {ReturnType<typeof normalizeProductInput>[]} */
  const imported = [];

  for (const entry of INTERCOPY_PINON_FUSOR_CATALOG) {
    ...
    console.log(`✓ ${entry.slug} → ${product.name} (${product.image_url})`);
  }
  ...
  console.log(`\nImportación piñon de fusor completada: ${created} nuevos, ${updated} actualizados.`);
}
