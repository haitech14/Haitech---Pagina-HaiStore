...
-- ---------------------------------------------------------------------------
-- Datos de ejemplo (pedidos demo para panel admin; solo si no hay pedidos)
-- ---------------------------------------------------------------------------
do $$
declare
  cust_id uuid;
  ...
  prod record;
begin
  if exists (select 1 from public.store_orders limit 1) then
    return;
  end if;
  ...
  if cust_id is null then
    insert into public.store_customers (email, full_name, company_name)
    values ('demo@haitech.pe', 'Cliente Demo', 'Empresa Demo SAC')
    returning id into cust_id;
  end if;

  select * into prod from public.products where id = 'ricoh-im-c3000' limit 1;
  ...
  values (
    ...
    'paid',
    'Tarjeta de crédito',
    coalesce(prod.price, 1890),
    0,
    coalesce(prod.price, 1890),
    round(coalesce(prod.price, 1890) * 3.7, 2),
    3.7,
    '{"line1":"Av. Javier Prado 1250","city":"San Isidro","region":"Lima","country":"PE"}'::jsonb,
    now() - interval '2 days',
    now() - interval '1 day',
    now() - interval '3 days'
  )
  ...
  select * into prod from public.products where id = 'hp-toner-58a' limit 1;
  ...
  values (
    ...
    coalesce(prod.price, 540) * 2,
    coalesce(prod.price, 540) * 2,
    round(coalesce(prod.price, 540) * 2 * 3.7, 2),
    3.7,
