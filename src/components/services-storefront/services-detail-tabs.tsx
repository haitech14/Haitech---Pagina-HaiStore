...
import { Headphones, PiggyBank, Printer, ShieldCheck } from 'lucide-react';

import type { ServiceCatalogItem } from '@/types/services-catalog';
import { cn } from '@/lib/utils';

const VALUE_ICONS = [PiggyBank, Printer, ShieldCheck, Headphones];
...
export function ServicesDetailTabs({ item, className }: ServicesDetailTabsProps) {
...
interface ServicesDetailValuePropsProps {
  item: ServiceCatalogItem;
  className?: string;
}

export function ServicesDetailValueProps({ item, className }: ServicesDetailValuePropsProps) {
  return (
    <aside
      className={cn(
      ...
      aria-labelledby="servicio-value-props-titulo"
    >
      <h2 id="servicio-value-props-titulo" className="text-base font-bold text-foreground">
