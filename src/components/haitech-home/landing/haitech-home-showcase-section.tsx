...
function BenefitCard({
  ...
  return (
    <article className="flex h-full flex-col items-center rounded-2xl border border-white/20 bg-white px-2.5 pb-4 pt-5 text-center shadow-[0_8px_28px_rgba(0,0,0,0.18)] sm:px-3 sm:pb-5 sm:pt-6 lg:px-2 lg:pt-5 xl:px-3">
      <div className="flex h-[72px] w-full items-center justify-center sm:h-[84px] lg:h-[76px] xl:h-[92px]">
        <img
          src={iconSrc}
          alt=""
          width={156}
          height={136}
          className="max-h-full max-w-[72%] object-contain sm:max-w-[78%]"
          loading="lazy"
          decoding="async"
        />
      </div>
      <h3
        className="mt-2 min-h-[2.4rem] max-w-[11.5rem] text-[11px] font-bold leading-[1.35] sm:mt-3 sm:min-h-[2.6rem] sm:max-w-[12.5rem] sm:text-[12px] lg:text-[11px] xl:text-[13px]"
        style={{ color: HAITECH_SHOWCASE_ACCENT }}
      >
        {title}
      </h3>
      <p className="mt-1.5 max-w-[12rem] flex-1 text-[10px] leading-[1.45] text-[#5C5C5C] sm:mt-2 sm:max-w-[13rem] sm:text-[11px] lg:text-[10px] xl:text-[12px]">
        {subtitle}
      </p>
      <span
        className="mt-4 block h-[3px] w-8 rounded-sm sm:mt-5 sm:w-9"
...
function BenefitsHeader() {
  ...
  return (
    <header className="mb-8 flex flex-col gap-5 sm:mb-10 lg:mb-12 lg:flex-row lg:items-end lg:justify-between lg:gap-8">
      <div className="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-center sm:gap-6 lg:gap-8">
        <div className="shrink-0">
          ...
          <h2
            ...
            className="mt-1 font-[family-name:var(--font-infobox)] text-[28px] font-bold leading-none tracking-tight text-white sm:text-[34px] lg:text-[40px]"
          >
            {header.titleBefore}
      ...
      <div className="shrink-0 lg:pb-1 lg:text-right">
        ...
        <span
          className="mt-1.5 block h-[3px] w-12 rounded-sm bg-white lg:ml-auto"
          aria-hidden="true"
        />
      </div>
    </header>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ul className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 sm:gap-4 md:grid-cols-3 lg:grid-cols-6 lg:gap-3 xl:gap-4">
