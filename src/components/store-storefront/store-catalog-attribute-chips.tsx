...
interface StoreCatalogAttributeChipsProps {
  ...
  /** Chip «Todas» a la izquierda (sin filtro de atributos rápido). */
  allOption?: {
    label?: string;
    count?: number;
    active: boolean;
...
export function StoreCatalogAttributeChips({
  ...
  if (attributes.length === 0 && !allOption) return null;

  const chipClass = (isActive: boolean, disabled = false) =>
    cn(
      'inline-flex h-8 max-w-[14rem] items-center rounded-full border px-3 text-xs font-semibold transition-colors',
  ...
  return (
    <div className={cn('min-w-0', className)}>
      <ul
        className="flex justify-center gap-1.5 overflow-x-auto pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        role="list"
        ...
        {allOption ? (
          <li className="shrink-0">
            <button
              ...
              aria-pressed={allOption.active}
              className={chipClass(allOption.active)}
            >
              <span className="truncate">{allOption.label ?? 'Todas'}</span>
            </button>
          </li>
        ) : null}
        {attributes.map((attr) => {
          ...
          return (
            <li key={attr.key} className="shrink-0">
              <button
                ...
                disabled={disabled}
                className={chipClass(isActive, disabled)}
              >
