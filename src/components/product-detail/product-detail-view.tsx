...
type DetailTab = 'description' | 'specs' | 'configuration' | 'consumables' | 'reviews';
...
export function ProductDetailView({ product, featuredMeta }: ProductDetailViewProps) {
  ...
  const tabs = useMemo(() => {
    ...
    return items;
  }, [equipmentSteps.length, showConsumablesTab, detail.reviews]);
  ...
  const heroGridClass =
    'grid gap-5 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:gap-7 xl:gap-9';
  ...
  return (
    <div className="bg-white">
      <div className="container py-3 sm:py-5">
        ...
        {showComboSection ? (
          <div className="mt-5 sm:mt-7 lg:max-w-[min(100%,42rem)]">
            {catalogLoading ? (
              <div
                className="overflow-hidden rounded-lg border border-border/60 bg-white"
                role="status"
                aria-live="polite"
                aria-label="Cargando consumibles recomendados"
              >
