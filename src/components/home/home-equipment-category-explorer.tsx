...
function matchesHomeEquipmentExplorerFilters(
  ...
  if (subcategoryId === 'todas') return true;

  return matchesHomeFeaturedEquipmentConditionFilter(
    product,
    subcategoryId as HomeFeaturedEquipmentConditionFilterId,
    categoryId,
  );
}
...
function HomeEquipmentSubcategoryPills({
  ...
  return (
    <div
      className="mb-4 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] sm:mb-5 [&::-webkit-scrollbar]:hidden"
      role="tablist"
        ...
        {subcategories.map((subcategory) => {
          ...
          return (
            <button
              ...
              className={cn(
                'inline-flex shrink-0 items-center justify-center rounded-full border px-3 py-1.5 text-[0.6875rem] font-semibold transition-colors sm:px-3.5 sm:py-1.5 sm:text-xs',
...
export function HomeEquipmentCategoryExplorer() {
  ...
  return (
    <div>
      ...
      {showSubcategories && activeSubcategoryDef?.href ? (
        <p className="mt-4 text-center sm:mt-5">
          <Link
            to={activeSubcategoryDef.href}
            className="text-xs font-semibold text-[#E30613] transition-colors hover:text-[#ff4d57] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E30613] focus-visible:ring-offset-2 sm:text-sm"
          >
            Ver {activeSubcategoryDef.label.toLowerCase()} en tienda →
          </Link>
        </p>
      ) : null}
    </div>
