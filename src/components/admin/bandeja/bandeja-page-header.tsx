...
interface BandejaPageHeaderProps {
  className?: string;
  onSelectConversation?: (id: string) => void;
}

export function BandejaPageHeader({ className, onSelectConversation }: BandejaPageHeaderProps) {
      ...
      >
        <div className="min-w-0 shrink-0">
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            Centraliza tus mensajes, tickets y conversaciones en todos los canales.
          </p>
        </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-2 sm:flex-row sm:items-center xl:max-w-2xl xl:justify-end">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            <Button
              ...
              aria-label="132 mensajes sin leer"
            >
      ...
      <CommandDialog open={commandOpen} onOpenChange={setCommandOpen}>
        ...
        <CommandList>
          ...
          <CommandGroup heading="Conversaciones">
            {BANDEJA_CONVERSATIONS.slice(0, 10).map((conversation) => (
              <CommandItem
                key={conversation.id}
                value={`${conversation.senderName} ${conversation.subject}`}
                onSelect={() => {
              ...
              >
                ...
                <span className="ml-2 truncate text-muted-foreground">{conversation.subject}</span>
              </CommandItem>
            ))}
          </CommandGroup>
        </CommandList>
